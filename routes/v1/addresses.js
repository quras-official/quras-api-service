...
function getAddress(address, res) {
	async.waterfall([
      ...
      var sqlTransaction = "SELECT txid, time as block_time, tx_type as type FROM tx_history WHERE claim_transaction_address = ? OR contract_transaction_from = ? OR contract_transaction_to = ? OR invocation_transaction_address = ? OR issue_transaction_address = ? OR issue_transaction_to = ? OR miner_transaction_address = ? OR miner_transaction_to = ?"
      
      try {
        var txsResult = connection.query(sqlUtxos, [address]);

        var txTransaction = connection.query(sqlTransaction, [address, address, address, address, address, address, address, address]);
